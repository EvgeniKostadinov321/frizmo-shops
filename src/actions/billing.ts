...
> {
  ...
  /* Нов модел (без trial/планове): без subscription = безплатен активен магазин.
     Task 8 пренаписва напълно към таксов статус (фактури + card-gate). */
  if (!sub) {
    return ok({ status: "free", plan: "free", currentPeriodEnd: null });
  }
  return ok({
    status: sub.status,
    plan: sub.plan,
