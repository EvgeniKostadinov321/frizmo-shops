...
export function ComplexityModeSwitcher({ mode, variant = "desktop", onChanged }: Props) {
  ...
  /* Списък с опциите — споделен между popover (десктоп) и инлайн (мобилно). */
  const options = MODE_META.map((m) => (
    <button
      key={m.value}
      type="button"
      role="menuitemradio"
  ...
  /* Мобилно: burger менюто вече е fullscreen → рендерираме опциите ИНЛАЙН
     (без popover, който се разливаше извън екрана). */
  if (variant === "mobile") {
    return (
      <div role="group" aria-label="Режим на сложност" className="flex flex-col gap-1">
  ...
  /* Десктоп: компактен бутон + popover дропдаун. */
  return (
    <div ref={ref} className="relative">
      <button
        ...
        aria-expanded={open}
        disabled={pending}
        className="flex h-9 items-center gap-1.5 rounded-control border border-surface-200 px-3 text-sm font-medium text-ink-700 transition-colors hover:bg-surface-100 hover:text-ink-900"
      >
        <Icon name="filter" size={16} className="text-ink-500" />
      ...
      {open && (
        <div
          role="menu"
          className="absolute right-0 z-50 mt-1 w-72 rounded-card border border-surface-200 bg-surface-0 p-1 shadow-float"
        >
          {options}
        </div>
      )}
