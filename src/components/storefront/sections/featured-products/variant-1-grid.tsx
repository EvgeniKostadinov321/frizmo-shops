...
import { discountPercent, ProductCard } from "../../product-card";
...
function Spotlight({ product, base }: { product: Product; base: string }) {
  ...
  return (
    ...
    >
      <span className="sf-frame relative aspect-4/3 overflow-hidden bg-(--sf-surface) md:aspect-auto md:min-h-96">
        {cover ? (
          <Image
      ...
      <span className="flex flex-col justify-center gap-4 p-8 sm:p-10">
        ...
        <span className="sf-cta mt-2 inline-flex h-12 w-fit items-center rounded-(--sf-radius) bg-(--sf-primary) px-7 font-medium text-(--sf-on-primary) transition-opacity group-hover:opacity-90">
          Виж продукта
        </span>
      </span>
    </Link>
...
function LargeCard({ product, base }: { product: Product; base: string }) {
  ...
  return (
    <Link
      href={`${base}/p/${product.slug}`}
      className="group flex h-full flex-col overflow-hidden rounded-(--sf-radius) border border-(--sf-border) bg-(--sf-surface-raised) shadow-(--sf-shadow) transition-[transform,box-shadow] duration-300 hover:-translate-y-0.5 hover:shadow-(--sf-shadow-hover)"
    >
      <span className="sf-frame relative min-h-72 flex-1 overflow-hidden bg-(--sf-surface)">
        {cover ? (
          <Image
            src={publicImageUrl(cover)}
            alt={product.name}
      ...
      <span className="flex flex-col gap-0.5 px-5 py-4">
        <span className="truncate text-xl leading-snug text-(--sf-text)" title={product.name}>
          {product.name}
        </span>
...
/** Вариант 1 — адаптивен grid: композицията следва броя продукти. */
export function FeaturedGrid({ data, products, ctx, tone }: FeaturedVariantProps) {
  ...
  if (count === 1) {
    ...
    /* Квадратни малки карти → 2×2 блокът (и голямата, която следва
       височината му) остават под един екран, не издуват секцията над 100vh. */
    body = (
      <div className="grid gap-3 md:grid-cols-2">
        <LargeCard product={first!} base={ctx.base} />
        <div className="grid grid-cols-2 gap-3">
          {rest.map((p) => (
            <ProductCard key={p.id} product={p} base={ctx.base} ratio="square" />
          ))}
        </div>
      </div>
    );
