...
- **2026-07-08 (тест-сесия) · `88f38a0` (dev)** — тестова опашка в ход.
  **Тествани и работещи (потвърдено от потребителя):** S5 аналитика, S12
  складови аларми, S6 ценови/наличност филтър, S7 bulk, **S14 back-in-stock**.
  Пътьом: TransitionLink loading state за филтри/пагинация; мобилен dashboard
  nav → burger меню в header-а; storefront `/products` едноредова филтърна лента
  (категория+подреди+цена дропдауни, ThemedDropdown с --sf-* токени); bulk
  ценова промяна вече отчита изчистени промо/сделки (транзакция); BG граматика
  одит + `docs/bulgarian-lang-guide.md` + `src/lib/plural.ts`; +5 тестови
  продукта със снимки (seed скриптове). Остава за тест: N11, S8, S10, S1, S4,
  N9, N12. Нищо не е push-нато към прод тази сесия (чака разрешение).
