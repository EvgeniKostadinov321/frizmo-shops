...
> **Купувачески профил ✅ имплементиран (12 задачи, среден обхват: поръчки+адреси+любими; toggle
> купувач/продавач на входа), commit-нат на `dev` локално, НЕ push-нат; e2e 2/2; чака ръчна
> проверка + push. Мулти-user екипи ПРЕМАХНАТИ от обхвата.** Оставаща пътна карта →
