...
- **2026-07-25 НОЩ — Landing v2 полиране + UX фиксове (до `32403d8`, dev+main).** След редизайна,
  по обратна връзка на живо: „хаос" → букви от хаос към ред (ChaosWord, замени scribble-а);
  hero+marquee запълват точно 100svh−хедъра (тъмната секция не наднича); „Преди/След" v3 —
  3 застъпени чат прозореца (Messenger/Viber/Instagram, баджове, „и още 9 чата") срещу табло
  със статистика + 2 поръчки с реални снимки; стъпковите карти с живи визуализации (курсор,
  реална снимка, hover сменя тема/публикува); PWA телефонът с начален екран + слайдващо push
  известие; регистрацията се събира без скрол на desktop (lg: свивания); телефонният номер
  премахнат от целия сайт (само имейл); реципрочна връзка „Продавам/Моят магазин" в /account
  (mobile + desktop); storefront: „Още магазини →" във footer-а + CatalogButton (стрелка +
  магазинче) в трите header варианта. Гейт зелен на всеки commit. ⚠️ Чака Vercel Promote.
- **2026-07-25 КЪСНО — LANDING V2 „Живият пазар" (пълен редизайн, Fable).** Тотално преобразяване на
  landing страницата (research + одит: `docs/superpowers/specs/2026-07-25-landing-v2-research-notes.md`):
  - **Hero v2:** витрината е СТАТИЧЕН showcase (Magnific продуктова фотография в `public/landing/`,
    НЕ база — решение: без демо seed на прод, до седмица идват 3 реални магазина) + CSS 3D tilt
    (изправя се на hover) + 3-та плаваща карта (★★★★★ отзив) + анимирано scribble подчертаване
    на „хаос" + категориен marquee + мобилен ред текст→витрина→чеклист.
  - **3 подписа:** (А) преоцветяваща витрина в bento — посетителят кликва палитра и мини-магазинът
    се преоцветява (`theme-recolor-demo.tsx`); (Б) „касова бележка" с fee слайдер в Цени
    (`fee-calculator.tsx`, смята през `src/lib/fee.ts`, перфориран clip-path); (В) шевица SVG
    hairline (`shevitsa-divider.tsx`, САМО hero + финален CTA).
  - **Ритъм:** „Преди/След" стана ТЪМНАТА секция (chat съобщенията се появяват стъпаловидно,
    typing точки, магазинът „изгрява"); DoneForYou се премести преди FAQ; стъпките имат свързваща
    пунктирана линия + publish toggle се „включва" на hover; нов CTA кадър (Magnific, златен час).
  - **„На живо" = РЕАЛНИ магазини:** `searchShops` + филтър „само с cover снимка" (крие e2e
    останки; самонапълва се с първите търговци; празна база → секцията липсва).
  - Фиксове от Playwright валидация: Motion изяждаше tilt-а (отделен слой), пунктираната линия
    рисуваше върху картите (painting order → z-10), mobile h-overflow 395px (text-6xl „Безплатно"
    + label wrap). Изтрити: pricing-card-spotlight, ThemeEditorMockup, стар cta-workshop.webp.
  - Гейт: 582/582 unit, e2e catalog 4/4, build зелен. Magnific: 375 кредита (5 генерации, 4 асета).
...
- **2026-07-25 (`9559bfe` dev+main) — ПРЕДСТАРТОВ МАРАТОН: auth фиксове + inv.bg + GDPR + Sentry +
  ...
  - **Auth production setup (ТЕСТВАН НА ЖИВО):** имейл потвърждение UX фикс (`signUp`→„Провери имейла",
    `signIn` различава непотвърден от грешна парола, `resendConfirmation`); callback поема code+token_hash.
    Root cause на „не мога да вляза": Supabase иска потвърждение. Site URL→frizmoshops.bg, Redirect URLs,
    **отделен prod Google OAuth client**. **Custom SMTP=Resend** (Supabase вграден лимит 2 имейла/час→429
    „зает имейл"; Resend→30/час). Прод база почистена (0 тестови). Виж [[auth-prod-setup]].
  - **inv.bg** (официални фактури за таксата) + **GDPR пакет** (consent при регистрация, експорт чл.20,
    легални текстове + cookies policy + международен трансфер клауза, данъчни конфликти fee_invoices НАП 10г);
    **Sentry** верифициран на живо. (виж съответните memory файлове).
  - **КУРИЕРИ v2 — разделяне + LIVE ЦЕНА (9 задачи, `a2cd1fe`..`33c3953`):** нова таблица
    `courier_delivery_options` (per shop+куриер+офис/адрес: резервна цена + праг безплатна). Таб „Куриери"
    поема ЦЯЛАТА куриерска настройка; таб „Доставка" = само pickup/local (courier тип премахнат).
    `CourierProvider.calculatePrice` (Speedy `/calculate` + Econt `mode:calculate`) → **live EUR цена** при
    checkout (безплатна>live>резервна). Валута = **EUR директно** (еврозона от 01.2026, БЕЗ BGN конверсия —
    сверено на живо). **Тествано с Playwright end-to-end:** Спиди офис в София → live 4,37€ (не резервните 5€).
    Живо хванати бъгове: Econt calculate иска `shipmentType:"PACK"` (иначе HTTP 517); default метод сийдваше
    невалиден courier тип; client/server зонов mismatch. Прод: таблицата създадена (таргетиран SQL, trgm 3/3
    запазени), 0 стари метода за миграция. Спец/план: `2026-07-24-courier-delivery-split-*.md`.
    Спиди/Еконт **тест ключове** свързани per-shop (за прод: реален договор). Виж [[courier-integration-feature]].
  - Освен това: address autocomplete в courier drawer-а + checkout layout преработка (вертикални секции).
  - ⏳ Остава: Vercel Production promote (git push ≠ авто-прод); жив тест на монетизацията (последно).

- **2026-07-24 — КРАЙ НА СЕСИЯ. Всичко синхронизирано (`26f1902`, dev=main=прод).** Тази сесия: ревизия
  „докъде сме" (`STATUS-2026-07-23.md`) + Спиди имейл изпратен + **4 одитни цикъла (37 поправки) + BL-01
  решено** — всичко на прод. Кодовата база значително уплътнена. **УТРЕ — остава (по решение на потребителя):**
  (1) **предстартов жив тест на монетизацията** (7 стъпки, паметта [[transaction-fee-monetization]]) на
  www.frizmoshops.bg с реална карта — единственото между нас и рекламите; (2) **`NEXT_PUBLIC_SITE_URL` на
  Vercel Production** = `https://www.frizmoshops.bg` + Redeploy (sitemap/OG/имейл линкове сочат стария fallback);
  (3) **Resend верификация** `shops@frizmoshops.bg`; (4) активиране на външни акаунти щом дойдат (Спиди тест
  ключове — чака отговор; Еконт prod; ePay); (5) **inv.bg + Sentry** (неписани модули); (6) дребен одитен
  дълг (~10 low находки в докладите, не спешни). Виж [[next-session-plan]], `docs/STATUS-2026-07-23.md`.
