...
> старите `/s/{slug}/account/*` пренасочват), commit-нат на `dev` локално, НЕ push-нат;
> e2e 4/4; чака ръчна проверка + push. Мулти-user екипи ПРЕМАХНАТИ от обхвата.** Оставаща пътна карта →
...
- **2026-07-13 (ПРОФИЛ ПОЛИРАНЕ: любими акаунт-базирани + десктоп layout + checkout дропдаун — commit-нат локално, НЕ push-нат)** —
  ...
  `store-products.spec.ts` флейква (потвърдено и на baseline — не е регресия). **ЧАКА push разрешение.**
...
- **2026-07-13 (ГЛОБАЛЕН КУПУВАЧЕСКИ ПРОФИЛ — имплементиран, commit-нат на `dev` локално, НЕ push-нат)** —
  ...
  **гард: акаунт с магазин → отказ**; (6) купувач след вход → `/account` (+ Google fallback);
  ...
  `buyer-account.spec.ts` към глобалния модел. `pnpm check` зелен (68 файла / 416 unit);
  e2e **4/4** (global-account 2 + buyer-account 2). **ЧАКА: ръчна проверка на живо + push
  разрешение** (вход като купувач → профил икона в каталога → `/account` → поръчки/любими;
  сърце на магазин; изтриване с „ИЗТРИЙ"). Заедно с per-магазин частта това е [[buyer-account-feature]].
