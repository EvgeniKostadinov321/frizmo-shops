...
1. ✅ **S1-01 🔴 ПОПРАВЕНО** — `payment_intents` индекс + `shopId` (db:push на dev
   готов; на прод ДОКАТО е чист — **P4-03**).
2. ✅ **S1-02 / S3-03 🟠 ПОПРАВЕНО** — ePay `URL_OK` + `?t=<token>`.
3. ✅ **S3-01 🔴 ПОПРАВЕНО** — ръчен cancel на `pending_payment` отменя и intent-а.
4. **P2-01 🔴⚙️** — ротирай прод DB парола + `SUPABASE_SECRET_KEY` (минаха през чата) — **оперативно, при Vercel/Supabase достъп**.

> Трите code блокера са поправени (commit локално, `pnpm check` зелено, 448 теста).
> План: `docs/superpowers/plans/2026-07-13-audit-blockers-fix.md`.
