...
4 находки: 1 HIGH · 2 MED · 1 LOW.
- **#3 N+1 featured-products** (MED) — ПОПРАВЕНО: една заявка + разпределение в JS.
- **#2 8 шрифта в critical path** (MED) — ПОПРАВЕНО: `preload:false` на 5-те
  storefront-темови шрифта → не тежат извън магазините.
- **#4 гол `<img>` в pwa-splash** (LOW) — ОТТЕГЛЕНО: съзнателно решение с
  eslint-disable коментар (статичен splash asset, извън оптимизация) → не е находка.
- **#1 storefront изцяло dynamic — нула кеш** (HIGH) — ОТЛОЖЕНО за собствен план
  (решение на потребителя 2026-07-07). `getPublicShop` вика `supabase.auth.getUser()`
  (cookies) → всяка storefront страница е SSR на всяка заявка, вкл. за анонимни
  купувачи. Пренаписването пипа 11 call-site-а + draft preview логиката; заслужава
  внимателен ISR + on-demand revalidation дизайн, не набързо между другото.
  **Известен архитектурен дълг — да се планира отделно след одитите.**
