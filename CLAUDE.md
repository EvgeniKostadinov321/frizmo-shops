...
**Какво следва:** `docs/remaining-roadmap.md` (актуалната карта; всички pure-code функции са завършени → следва външна работа) · историческият `docs/superpowers/plans/2026-07-07-post-audit-roadmap.md` е вече изпълнен
...
* `.env.local` ключове: Supabase URL/publishable/secret, `DATABASE_URL` (:6543, transaction pooler) / `DATABASE_URL_MIGRATIONS` (:5432, session), `NEXT_PUBLIC_HERE_API_KEY`, `RESEND_API_KEY`, `EMAIL_FROM` (изпращащ адрес на имейлите; опц., fallback `Frizmo Shops <shops@frizmo.bg>` — единственият Verified домейн в Resend; frizmoshops.bg НЕ се добавя — free план = 1 домейн, Pro е излишен разход за козметика, решение 2026-07-25), VAPID двойка (`VAPID_PRIVATE_KEY` + `NEXT_PUBLIC_VAPID_PUBLIC_KEY`) + `VAPID_SUBJECT`, `NEXT_PUBLIC_SITE_URL` (базов URL за имейл линкове; fallback е `frizmo-shops.vercel.app`), `PLATFORM_ADMIN_EMAILS` (comma-separated; дава достъп до /admin), `CRON_SECRET` (таен низ; гард за Vercel Cron — abandoned cart route; нужен и във Vercel prod + Redeploy), Sentry: `NEXT_PUBLIC_SENTRY_DSN` + `SENTRY_DSN` (един и същ DSN, публичен — проект frizmo-shops в org frizmo-tech-ltd; липсата = Sentry тихо изключен) + опц. `SENTRY_AUTH_TOKEN` (таен, само за source map upload при билд — без него билдът минава с warning), `INV_BG_TOKEN` (таен; официални inv.bg фактури за месечната такса — издават се при invoice.paid webhook, ПРОД-ONLY чрез `VERCEL_ENV==="production"` гард; липса → само Stripe таксата без официална фактура; `INV_BG_FORCE=1` форсира издаване извън прод за жив тест).
